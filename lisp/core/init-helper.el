...
(defun riven/package-vc-upgrade-all ()
  "Upgrade all VC packages recorded in `package-vc-selected-packages`."
  (interactive)
  (if (not (boundp 'package-vc-selected-packages))
      (message "`package-vc-selected-packages` is not available in this Emacs.")
    (let ((pkgs (mapcar #'car package-vc-selected-packages))
          (ok 0)
          (fail 0))
      (if (null pkgs)
          (message "No VC packages to upgrade.")
        (dolist (pkg pkgs)
          (condition-case err
              (progn
                (package-vc-upgrade pkg)
                (setq ok (1+ ok)))
            (error
             (setq fail (1+ fail))
             (message "Failed to upgrade %s: %s" pkg (error-message-string err)))))
        (message "VC upgrade done: %d success, %d failed." ok fail)))))
