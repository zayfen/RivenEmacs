...
(setq
 ;; Set a theme for RivenEmacs, supported themes include these from `doom-themes'
 ;; and `modus-themes'.
 rivenemacs-theme 'doom-one ; `doom-one' is a dark theme, `doom-one-light' is the light one
...
(+deferred!
 ...
 ;; `re-prog' module). You can use `+lsp-auto-enable' instead to automatically
 ;; enable LSP mode in supported modes (from the `re-lsp' module).
 (+eglot-auto-enable))
...
(setq browse-url-chrore-program (or (executable-find "brave") (executable-find "chromium")))

;; Module: `re-natural-langs' -- Package: `spell-fu'
(with-eval-after-load 'spell-fu
  ;; We can use RivenEmacs' helper macro `+spell-fu-register-dictionaries'
  ;; to enable multi-language spell checking.
  (+spell-fu-register-dictionaries "en" "fr"))

;; Module: `re-rss' -- Package: `elfeed'
...
(with-eval-after-load 'mu4e
  ...
  ;; (setq mail-personal-alias-file (concat rivenemacs-config-dir "private/mail-aliases.mailrc"))
  ...
  ;; Use RivenEmacs' `+mu4e-register-account' helper function to register multiple accounts
...
;; Module: `re-notes' -- Package: `org-roam'
