...
(use-package sly
  :straight t
  :custom
  (sly-mrepl-history-file-name (+directory-ensure rivenemacs-local-dir "sly/mrepl-history.el"))
...
(use-package elisp-mode
  ...
  :after rivenemacs-loaded ; prevent elisp-mode from being loaded too early
  :init
  (+map-local! :keymaps '(emacs-lisp-mode-map lisp-interaction-mode-map ielm-map lisp-mode-map racket-mode-map schere-mode-map)
    "p" #'check-parens)
  :config
...
(use-package re-elisp-extras
  :after elisp-mode rivenemacs-loaded
  :demand t
  :config
  (+elisp-indent-setup)
